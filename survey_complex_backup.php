...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_survey'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO survey_submissions (survey_id, customer_id, asset_id, submitted_by, status) 
            VALUES (?, ?, ?, ?, 'completed')
        ");
        ...
        foreach ($_POST as $key => $value) {
            if (strpos($key, 'question_') === 0) {
                ...
                $ins = $pdo->prepare("
                    ...
                    (submission_id, question_id, response_text) 
                    VALUES (?, ?, ?)
                ");
    ...
    if ($submission_id && $customer_id && $phone) {
        try {
            ...
            if ($sms_result['success']) {
                // ذخیره اطلاعات ارسال پیامک (اگر ستون وجود دارد)
                try {
                    $stmt = $pdo->prepare("
                    ...
                    // ستون‌های SMS ممکن است وجود نداشته باشند
                }
