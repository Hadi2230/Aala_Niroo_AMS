<?php
/**
 * test_complete_workflow.php - تست کامل سیستم گردش کار
 */
...
<head>
    <meta charset='UTF-8'>
    <title>تست سیستم گردش کار</title>
...
echo "<h1 class='text-center mb-4'>تست کامل سیستم گردش کار درخواست‌ها</h1>";
...
try {
    ...
    if ($request_id) {
        ...
        echo "<p class='error'>❌ خطا در ایجاد درخواست</p>";
    }
} catch (Exception $e) {
...
// 5. تست نمایش درخواست‌ها
echo "<div class='test-section'>
    <h3>5. تست نمایش درخواست‌ها</h3>";
try {
    $stmt = $pdo->prepare("
        SELECT r.*, 
...
try {
    ...
    if (empty($workflows)) {
        echo "<p class='warning'>⚠️ هیچ گردش کاری یافت نشد</p>";
    } else {
        echo "<p class='success'>✅ " . count($workflows) . " مرحله گردش کار یافت شد</p>";
        foreach ($workflows as $workflow) {
            echo "<div class='alert alert-secondary'>
                <strong>درخواست:</strong> {$workflow['request_number']}<br>
                <strong>مرحله:</strong> {$workflow['step_order']}<br>
                <strong>ارجاع شده به:</strong> {$workflow['full_name']} ({$workflow['username']})<br>
                <strong>واحد:</strong> {$workflow['department']}<br>
                <strong>وضعیت:</strong> {$workflow['status']}<br>
            </div>";
        }
    }
} catch (Exception $e) {
    echo "<p class='error'>❌ خطا در نمایش گردش کار: " . $e->getMessage() . "</p>";
}
echo "</div>";
...
echo "<div class='test-section'>
    ...
    <div class='d-grid gap-2 d-md-block'>
        ...
        <a href='request_tracking_final.php' class='btn btn-success'>صفحه پیگیری درخواست‌ها</a>
        <a href='test_request_workflow.php' class='btn btn-info'>تست جزئیات</a>
    </div>
</div>";
