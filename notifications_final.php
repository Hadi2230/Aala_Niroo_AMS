...
try {
    $stats = [
        ...
        'ticket' => $pdo->query("SELECT COUNT(*) FROM notifications WHERE user_id = " . $_SESSION['user_id'] . " AND type = 'تیکت'")->fetchColumn(),
        'maintenance' => $pdo->query("SELECT COUNT(*) FROM notifications WHERE user_id = " . $_SESSION['user_id'] . " AND type = 'تعمیرات'")->fetchColumn(),
        'message' => $pdo->query("SELECT COUNT(*) FROM notifications WHERE user_id = " . $_SESSION['user_id'] . " AND type = 'پیام'")->fetchColumn()
    ];
} catch (Exception $e) {
...
<head>
    ...
    <style>
        ...
        .notification-item.تیکت { border-left-color: #3498db; }
        .notification-item.تعمیرات { border-left-color: #f39c12; }
        .notification-item.پیام { border-left-color: #9b59b6; }
        .notification-item.سیستم { border-left-color: #95a5a6; }
                            ...
                            <div class="col-md-4">
                                <form method="GET" class="d-flex">
                                    ...
                                    <select class="form-select" name="type" onchange="this.form.submit()">
                                        ...
                                        <option value="تیکت" <?php echo $type_filter === 'تیکت' ? 'selected' : ''; ?>>تیکت</option>
                                        <option value="تعمیرات" <?php echo $type_filter === 'تعمیرات' ? 'selected' : ''; ?>>تعمیرات</option>
                                        <option value="پیام" <?php echo $type_filter === 'پیام' ? 'selected' : ''; ?>>پیام</option>
                                        <option value="سیستم" <?php echo $type_filter === 'سیستم' ? 'selected' : ''; ?>>سیستم</option>
                                    </select>
                                </form>
                            </div>
                        </div>
                                            ...
                                            <div class="d-flex justify-content-between align-items-start">
                                                <div class="flex-grow-1">
                                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                                        ...
                                                        <div>
                                                            <span class="badge type-badge bg-<?php echo $notification['type'] === 'تیکت' ? 'primary' : ($notification['type'] === 'تعمیرات' ? 'warning' : ($notification['type'] === 'پیام' ? 'info' : 'secondary')); ?>">
